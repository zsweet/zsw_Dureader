...
class BRCDataset(object):
    ...
    """
    def __init__(self, max_para_num, max_p_len, max_q_len,max_train_sample_num,train_files=[], test = False):
        ...
        if 1:
            ...
            if train_files:
                for train_file in train_files:
                    ...
                    dataset += self._load_dataset(train_file, train=True, test = test)
            if test == False:
                random.shuffle(dataset)
                self.train_set, self.dev_set, self.test_set = self.split_dataset_proportions(dataset)
            else:
                self.test_set = dataset
                self.logger.info('Test set size: {} documents.'.format(len(self.test_set)))
            self.logger.info('Train set size: {} documents.'.format(len(self.train_set)))

    def compute_rank(self, answer, fake_answer):
        para_infos = []
        ...
        for i in range(self.max_para_num):
            fake_passage_tokens = []
    ...
    def _load_dataset(self, data_path, train=False, test = False):
        ...
        """
        Count = 0
        max_para = self.max_para_num
        is_not_selected_counter = 0
        with open(data_path) as fin:
            ...
            for lidx, line in enumerate(fin):
                ...
                sample1['question_tokens'] = sample['segmented_question']
                sample1['question_id'] = sample['question_id']
                ...
                for d_idx, doc in enumerate(sample['documents']):
                    ...
                    if train:
                        if len(rank) == 0:
                            ...
                            Count+=1
                    sample2['question_tokens'] = sample['segmented_question']
                    sample2['question_id'] = sample['question_id']

                    data_set.append(sample2)
            self.logger.info('共筛选出{}个无效doc 最终document个数为：{},设置的最大doc个数为：{}'.format(is_not_selected_counter,Count,self.max_train_sample_num))
        return data_set
    ...
    def _one_mini_batch(self, data,  indices, pad_id):
        ...
        for sidx, sample in enumerate(batch_data['raw_data']):
            for pidx in range(max_para):
                ...
                batch_data['question_id'].append(sample['question_id'])
                batch_data['doc_idx'].append(sample['doc_idx'])
