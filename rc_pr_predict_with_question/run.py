...
def parse_args():
    ...
    train_settings.add_argument('--batch_size', type=int, default=25,
    ...
    path_settings.add_argument('--vocab_dir', default='../data/vocab/demo',
                               help='the dir to save vocabulary')
    path_settings.add_argument('--model_dir', default='../data/models/demo',
                               help='the dir to store models')
    path_settings.add_argument('--sys_dir', default='../data/models/sys',
                               help='the dir to store models')
    path_settings.add_argument('--result_dir', default='../data/results/demo',
    ...
    path_settings.add_argument('--brc_dir', default='../data/baidu',
                               help='the dir with preprocessed baidu reading comprehension data')
    path_settings.add_argument('--vocab_dir', default='../data/vocab/search_emb',
                               help='the dir to save vocabulary')
    path_settings.add_argument('--model_dir', default='../data/models/search_emb1',
                               help='the dir to store models')
    path_settings.add_argument('--sys_dir', default='../data/models/sys',
                               help='the dir to store models')
    ...
    path_settings.add_argument('--dev_files', nargs='+',
                               ...
                               help='list of files that contain the preprocessed dev data')
    path_settings.add_argument('--test_files', nargs='+',
                               default=['../data/test1set/preprocessed/zhidao.test1.json'],
                               help='list of files that contain the preprocessed test data')
    path_settings.add_argument('--brc_dir', default='../data/baidu',
                               help='the dir with preprocessed baidu reading comprehension data')
...
def prepare(args):
    ...
    unfiltered_vocab_size = vocab.size()
    vocab.filter_tokens_by_cnt(min_cnt=2)
    vocab1.filter_tokens_by_cnt(min_cnt=34)
    filtered_num = unfiltered_vocab_size - vocab.size()
    ...
    logger.info('Assigning embeddings...')
    vocab.load_pretrained_embeddings('word2vec_size300_mincount1.txt')
    vocab1.load_pretrained_embeddings('word2vec_size300_mincount1.txt')
    #vocab.randomly_init_embeddings(300)
    #vocab1.randomly_init_embeddings(300)
    logger.info('Saving vocab...')
    brc_data.convert_to_ids(vocab, vocab1)
...
#    with open(args.vocab_dir+'/vocab.data', 'rb') as fin:
 #       vocab2 = pickle.load(fin)
    ...
    rc_model = RCModel(vocab,vocab, args)
...
def evaluate(args):
    ...
    brc_data = BRCDataset(args.max_p_num, args.max_p_len, args.max_q_len, args.max_a_len,dev_files=args.dev_files)
    logger.info('Converting text into ids...')
    brc_data.convert_to_ids(vocab)
...
#     with open(os.path.join(args.vocab_dir, 'vocab.data'), 'rb') as fin:
#         vocab = pickle.load(fin)
#     with open(os.path.join('../data/vocab/sys', 'tar_vocab.data'), 'rb') as fin:
#         vocab1 = pickle.load(fin)
#     with open(os.path.join('../data/vocab/sys', 'vocab.data'), 'rb') as fin:
#         vocab2 = pickle.load(fin)
#     assert len(args.test_files) > 0, 'No test files are provided.'
#
#     brc_data = BRCDataset(args.max_p_num, args.max_p_len, args.max_q_len,args.max_a_len,
...
#
#     rc_model = RCModel(vocab,vocab1,vocab2,args)
#     logger.info('Restoring the model...')
#     rc_model.restore(model_dir=args.model_dir , model_prefix=args.algo + 'syst', sys_dir=args.sys_dir, sys_prefix= args.algo + 'sys')
#     logger.info('Predicting answers for test set...')
#     test_batches = brc_data.gen_mini_batches('test', args.batch_size,
#                                              pad_id=vocab.get_id(vocab.pad_token), shuffle=False)
#     rc_model.evaluate1(test_batches,
#                       result_dir=args.result_dir, result_prefix='test.predicted')
#
