...
class MatchLSTMLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):  ###passage_encodes, question_encodes 300维
        """
...
class AttentionFlowMatchLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            sim_matrix = tf.matmul(passage_encodes, question_encodes, transpose_b=True)  ## T*300  300*J  =>   T*J  context_number*query_number

            context2question_attn = tf.matmul(tf.nn.softmax(sim_matrix, -1), question_encodes)   ###(按照每行softmax)   T*J  , J*300   => T*300
