...
class Vocab(object):
    ...
    def filter_tokens_by_cnt(self, min_cnt):
        ...
        unfiltered_tokens =  [token for token in self.token2id if self.token_cnt[token] < min_cnt]
        logger = logging.getLogger("brc")
        logger.info(
            'vocab : filtered_tokens list length:{} list:{}'.format(len(filtered_tokens), filtered_tokens))
        logger.info('vocab : unfiltered_tokens list length:{} list:{}'.format(len(unfiltered_tokens),unfiltered_tokens))
