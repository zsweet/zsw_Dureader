...
class BRCDataset(object):
    ...
    def __init__(self, max_p_num, max_p_len, max_q_len, max_a_len,
        ...
        if dev_files:
            for dev_file in dev_files:
                self.dev_set += self._load_dataset(dev_file, yes_no=True, entity = False, td= True)
    ...
    def gen_pr_dict(self):
        pr_dict = {}
        with open('pr.json') as fin:
            for lidx, line in enumerate(fin):
                sample = json.loads(line.strip())
                qid = sample['question_id']
                rank = sample['best']
                docnum = sample['doc_num']
                if not qid in pr_dict:
                    pr_dict[qid] = {}
                pr_dict[qid][docnum] = rank
        self.logger.info('dict is ready')
        print('dict_ready')
        return pr_dict

    def _load_dataset(self, data_path, train=False, yes_no = False, entity = False, td = False):
        """
        Loads the dataset
        Args:
            data_path: the data file to load
        """
        wrong_para_num_counter = 0
            ...
            for lidx, line in enumerate(fin):
                ...
                for d_idx, doc in enumerate(sample['documents']):
                    if train:
                        ...
                        '''
                        qid = sample['question_id']
                        rank = self.pr_dict[qid][d_idx]
                        if rank >= len(doc['segmented_paragraphs']):
                            self.logger.info('{}  {}'.format(qid,rank))
                            wrong_para_num_counter+=1
                            para_infos = []
                            for para_tokens in doc['segmented_paragraphs']:
                                question_tokens = sample['segmented_question']
                                common_with_question = Counter(para_tokens) & Counter(question_tokens)
                            ...
                            sample['passages'].append({'passage_tokens': doc['segmented_paragraphs'][rank]})
                data_set.append(sample)
        self.logger.info('wrong paragraph number counter:{0}'.format(wrong_para_num_counter))
        return data_set
