...
void SortVocab() {
  ...
  for (a = 1; a < size; a++) { // Skip </s>
    ...
    if (vocab[a].cn < min_count) {
      ...
      printf("次数：%lld,单词：%s，字符个数：%d\n",vocab[a].cn,vocab[a].word,vocab[a].character_size);
...
void TrainModel(){
  ...
  printf("LearnVocabFromTrainFile is excuted!\n");
  ReadComponent();
  printf("ReadComponent is excuted!\n");
  LearnCharComponentsFromFile();
  printf("LearnCharComponentsFromFile is excuted!\n");
  if (output_word[0] == 0) return;
  InitNet();
  printf("InitNet is excuted!\n");
  if (negative > 0) InitUnigramTable();
  printf("InitUnigramTable is excuted!\n");
  start = clock();
  for (a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread, (void *)a);
  printf("pthred_create TrainModelThread is excuted!\n");
...
int main(int argc, char **argv) {
  ...
  if(argc == 1){
    printf("WORD VECTOR estimation toolkit v 0.1b\n\n");
      ...
      printf("\t\tThis will discard words that appear less than <int> times; default is 2\n");
      printf("\t-alpha <float>\n");
